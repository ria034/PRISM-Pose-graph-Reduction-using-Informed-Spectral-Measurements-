...
import mac.utils.enhanced_fiedler as enhanced_fiedler
...
class MAC:
    ...
    def __init__(self, fixed_edges, candidate_edges, num_nodes,
                ...
                fiedler_module="enhanced", # "standard" or "enhanced"
        ...
        self.fiedler_module = fiedler_module
    ...
    def evaluate_objective(self, x):
        ...
        """
        if self.fiedler_module == "standard":
            return fiedler.find_fiedler_pair(L=self.laplacian(x), method=self.fiedler_method, tol=self.fiedler_tol)[0]
        elif self.fiedler_module == "enhanced":
            return enhanced_fiedler.find_fiedler_pair(L=self.laplacian(x))[0]
        else: 
            raise ValueError(f"Invalid fiedler_module: {self.fiedler_module}")
    ...
    def problem(self, x, cache=None):
        ...
        if self.fiedler_module == "standard":
            f, fiedler_vec, Qnew = fiedler.find_fiedler_pair(L=self.laplacian(x), X=Q) # original MAC
        elif self.fiedler_module == "enhanced":
            f, fiedler_vec = enhanced_fiedler.find_fiedler_pair(L=self.laplacian(x), X0=Q) # Enhanced MAC
        else: 
            raise ValueError(f"Invalid fiedler_module: {self.fiedler_module}")
