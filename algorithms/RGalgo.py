import os
os.add_dll_directory(r'C:\Users\neilk\.vscode\SE-Sync\C++\build\lib\RelWithDebInfo')
os.add_dll_directory(r'C:\Users\neilk\miniconda3\envs\sesync\Library\bin')
import matplotlib
...
# conda activate sesync
# cd C:\Users\neilk\.vscode\MAC-Explorer\examples
# python proof.py manhattanOlson3500.g2o
# python "C:\Users\neilk\.vscode\MAC-Explorer\examples\proof.py" "C:\Users\neilk\.vscode\MAC-Explorer\data\kitti_02.g2o"


#inverse persion, 1/persision
# mim weight weights
# breath first search, but want to the depth as shallow as pooissble, return the graph
# red black trees
...
def minimum_diameter_spanning_tree(candidate_edges, num_nodes):
    G = nx.Graph()
    G.add_nodes_from(range(num_nodes))
    for e in candidate_edges:
        G.add_edge(e.i, e.j, weight= 1.0 / e.weight)  # using precision


    # Find the center: node with smallest max-distance to any other node
    lengths = dict(nx.all_pairs_dijkstra_path_length(G, weight='weight'))
    center = min(range(num_nodes), key=lambda v: max(lengths[v].values()))

    # Shortest path tree from center (minimizes uncertainty along each path)
    spt_paths = nx.single_source_dijkstra_path(G, center, weight='weight')

    tree_edges = set()
    for path in spt_paths.values():
        for a, b in zip(path, path[1:]):
            tree_edges.add((min(a, b), max(a, b)))

    x = np.zeros(len(candidate_edges))
    for k, e in enumerate(candidate_edges):
        if (min(e.i, e.j), max(e.i, e.j)) in tree_edges:
            x[k] = 1.0
    return x
...
if __name__ == '__main__':
    ...
    #x_mst = minimum_diameter_spanning_tree(all_edges,num_poses)
    precision_weights = np.array([e.weight for e in all_edges])
    x_mst = minimum_diameter_spanning_tree(all_edges, num_poses)
