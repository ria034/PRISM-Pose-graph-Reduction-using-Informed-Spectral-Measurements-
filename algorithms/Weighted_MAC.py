...
from mac.solvers.MAC_weighted import MAC as MAC_weighted
...
sesync_lib_path = r"E:\Git\SE-Sync\C++\build\lib\Release"
...
if __name__ == '__main__':
    ...
    for pct in percents:
        ...
        mac_mst_time.append(mst_time)
        mac_mst_nearest_time.append(mac_solve + mst_time + n_time)
        mac_mst_madow_time.append(mac_solve + mst_time + m_time)

    # ---- Baselines ----
    MAC_original_nearest_results = []
    MAC_original_madow_results   = []
    odom, candidates = split_edges(all_edges)
    mac_original   = MAC_weighted(fixed_edges=odom, candidate_edges=candidates, num_nodes=num_poses, fiedler_method="tracemin_lu")
    ...
    start = timer()
    x_MST_Madow_eff = build_MST(all_edges, weights_enhanced_MAC, num_poses)
    end = timer()
    mst_eff = end - start
    ...
    mac_effrMadow   = MAC_weighted(fixed_edges=mst_edges, candidate_edges=non_mst_edges, num_nodes=num_poses, fiedler_method="tracemin_lu")
    ...
    for pct_effrMadow in percent_lc:
        ...
        effrMadow_round_time.append(mst_eff + mac_eff + round_eff)
